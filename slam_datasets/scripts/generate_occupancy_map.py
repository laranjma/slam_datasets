...
DEFAULT_CONFIG = {
    "raw_log": None,
    "relations_log": None,
    "output": Path("occupancy_grid_with_trajectory.png"),
    "resolution": 0.05,
    "max_range": 12.0,
    "beam_step": 2,
    "scan_step": 1,
...
def load_config(config_path: Path) -> Dict[str, Any]:
    """Load script configuration from a simple YAML key-value config file."""
    ...
    path_keys = {"raw_log", "relations_log", "output"}
    config: Dict[str, Any] = {}
    with config_path.open("rt", encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            if not value or value.lower() in {"null", "none"}:
                config[key] = None if key in path_keys else ""
            ...
            if key in path_keys and isinstance(config[key], str):
...
def parse_args() -> argparse.Namespace:
    default_config_path = Path(__file__).resolve().parents[2] / "config" / "generate_occupancy_map.yaml"
    parser = argparse.ArgumentParser(
        description=(
            "Generate an occupancy grid map from a CARMEN raw log using a "
    ...
    parser.add_argument(
        ...
        help="Path to YAML file containing all script parameters.",
    )
    cli_args = parser.parse_args()

    config_values = DEFAULT_CONFIG.copy()
    config_values.update(load_config(cli_args.config))
    config_values["config"] = cli_args.config

    args = argparse.Namespace(**config_values)
    if args.raw_log is None:
        parser.error("'raw_log' is required in the YAML config file.")
    return args
